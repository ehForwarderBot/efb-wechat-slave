...
fromitchat import utils as itchat_utils
...
def weechat_string_unescape(content: str) -> str:
    """
    Unescape a Weechat HTML string.
...
def generate_message_uid(messages: List[ircpy.SentMessage]) -> MessageID:
...
def message_id_to_dummy_message(message_uid: List[str], channel: 'WeechatChannel') -> ircpy.SentMessage:
    """
    Generate a ircpy.SentMessage object with minimum identifying information.
    This is generally used to recall messages using IRCPY's API without the message object
    ...
    d = {
        'receiver': channel.chats.get_ircpy_chat_by_uid(puid),
        'id': m_id,
        'local_id': l_id
    }
    return ircpy.SentMessage(d)
