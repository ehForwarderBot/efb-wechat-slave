...
import ircpy
fromircpy import ResponseError
fromircpy.utils import PuidMap


class WeechatChannel(SlaveChannel):
    """
    EFB Channel - Weechat Slave Channel
    Based on ircpy (itchat), Weechat Web Client

    Author: Eana Hufwe <https://github.com/blueset>
    """

    channel_name = "Weechat Slave"
    channel_emoji = "💬"
    channel_id = ModuleID('blueset.weechat')
    ...
    logger: logging.Logger = logging.getLogger(
        "plugins.%s.WeechatChannel" % channel_id)
    ...
    translator = translation("efb_weechat_slave",
                             resource_filename('efb_weechat_slave', 'locale'),
                             fallback=True)
    ...
    SYSTEM_ACCOUNTS: Final = {
        # TRANSLATORS: Translate this to the corresponding display name of the Weechat system account. Guessed names are not accepted.
        ...
        'voip': _('voip'),
        # TRANSLATORS: Translate this to the corresponding display name of the Weechat system account. Guessed names are not accepted.
        'blogappwaysheen': _('blogappwaysheen'),
        # TRANSLATORS: Translate this to the corresponding display name of the Weechat system account. Guessed names are not accepted.
        'waysheen': _('waysheen'),
        # TRANSLATORS: Translate this to the corresponding display name of the Weechat system account. Guessed names are not accepted.
        'brandsessionholder': _('brandsessionholder'),
        # TRANSLATORS: Translate this to the corresponding display name of the Weechat system account. Guessed names are not accepted.
        'waysheenreminder': _('waysheenreminder'),
        # TRANSLATORS: Translate this to the corresponding display name of the Weechat system account. Guessed names are not accepted.
    ...
    def console_qr_code(self, uuid, status, qrcode=None):
        ...
        if status == 201:
            ...
            qr += "\n"
            qr_url = "https://login.waysheen.qq.com/l/" + uuid
            ...
            qr += "\n" + self._("If the QR code was not shown correctly, please visit:\n"
                                "https://login.waysheen.qq.com/qrcode/{0}").format(uuid)
            return self.logger.log(99, qr)
    ...
    def exit_callback(self):
        ...
        if not getattr(coordinator, 'master', None):
            raise Exception(
                self._("Web Weechat logged your account out before master channel is ready."))
        ...
        msg = Message(
            ...
            deliver_to=coordinator.master,
            text=self._(
                "Weechat server has logged you out. Please log in again when you are ready."),
    ...
    def send_message(self, msg: Message) -> Message:
        ...
        chat: ircpy.Chat = self.chats.get_ircpy_chat_by_uid(msg.chat.uid)

        # List of "SentMessage" response for all messages sent
        r: List[ircpy.SentMessage] = []
        self.logger.info("[%s] Sending message to Weechat:\n"
                         "uid: %s\n"
                         "UserName: %s\n"
                         "NickName: %s\n"
                         "Type: %s\n"
        ...
        try:
            chat.mark_as_read()
        except ircpy.ResponseError as e:
        ...
        if send_text_only or msg.type in [MsgType.Text, MsgType.Link]:
            if isinstance(msg.target, Message):
                ...
                if max_length > 0:
                    ...
                    tgt_text = ""
                if isinstance(chat, ircpy.Group) and not isinstance(msg.target.author, SelfChatMember):
            ...
            if convert_to == "image/gif":
                ...
                try:
                    ...
                    file.seek(0)
                    self.logger.debug(
                        "[%s] Sending %s (image) to Weechat.", msg.uid, msg.path)
            ...
            self.logger.info("[%s] Sending %s to Weechat\nFileName: %s\nPath: %s\nFilename: %s",
        ...
        self.logger.debug(
            'Weechat message is assigned with unique ID: %s', msg.uid)
        return msg
    ...
    def get_chat_picture(self, chat: Chat) -> BinaryIO:
        uid = chat.uid
        if uid in ircpy.Chat.SYSTEM_ACCOUNTS:
            ircpy_chat: ircpy.Chat = ircpy.Chat(
                ircpy.utils.wrap_user_name(uid), self.bot)
        else:
            ircpy_chat = ircpy.utils.ensure_one(self.bot.search(puid=uid))
        f: BinaryIO = None  # type: ignore
        try:
            f = tempfile.NamedTemporaryFile(suffix='.jpg')  # type: ignore
            data = ircpy_chat.get_avatar(None)
            if not data:
                raise EFBOperationNotSupported()
            f.write(data)
            f.seek(0)
    ...
    @extra(name=_("Show chat list"),
           desc=_("Show a list of chats from Weechat.\n"
                  "Usage:\n    {function_name} [-r]\n"
                  "    -r: Refresh list"))
    def get_chat_list(self, param: str = "") -> str:
        ...
        l: List[ircpy.Chat] = self.bot.chats(refresh)

        msg = self._("Chat list:") + "\n"
        for i in l:
            alias = ews_utils.weechat_string_unescape(getattr(i, 'remark_name', '') or
                                                     getattr(i, 'display_name', ''))
            name = ews_utils.weechat_string_unescape(i.nick_name)
            display_name = "%s (%s)" % (
                alias, name) if alias and alias != name else name
            chat_type = "?"
            if isinstance(i, ircpy.MP):
                # TRANSLATORS: Acronym for MP accounts
                chat_type = self._('MP')
            elif isinstance(i, ircpy.Group):
                # TRANSLATORS: Acronym for groups
                chat_type = self._('Gr')
            elif isinstance(i, ircpy.User):
    ...
    def set_alias(self, r_param: str = "") -> str:
        ...
        if not isinstance(chat, ircpy.User):
    ...
    @extra(name=_("Log out"),
           desc=_("Log out from Weechat and try to log in again.\n"
                  "Usage: {function_name}"))
    ...
    def authenticate(self, qr_reload, first_start=False):
        ...
        with coordinator.mutex:
            self.bot: ircpy.Bot = ircpy.Bot(cache_path=str(efb_utils.get_data_path(self.channel_id) / "ircpy.pkl"),
                                          qr_callback=qr_callback,
                                          logout_callback=self.exit_callback,
                                          user_agent=self.flag('user_agent'),
                                          start_immediately=not first_start)
            self.bot.enable_puid(
                efb_utils.get_data_path(self.channel_id) / "ircpy_puid.pkl",
                self.flag('puid_logs')
            )
            self.done_reauth.set()
            if hasattr(self, "slave_message"):
                self.slave_message.bot = self.bot
                self.slave_message.weechat_msg_register()
    ...
    def add_friend(self, username: str = None, verify_information: str = "") -> str:
        ...
        except ircpy.ResponseError as r:
    ...
    def get_chats(self) -> List[Chat]:
        """
        Get all chats available from Weechat
        """
    ...
    def _bot_send_msg(self, chat: ircpy.Chat, message: str) -> ircpy.SentMessage:
        try:
            return chat.send_msg(message)
        except ircpy.ResponseError as e:
            e = self.substitute_known_error_reason(e)
            raise EFBMessageError(self._("Error from Web Weechat while sending message: [{code}] {message}")
                                  .format(code=e.err_code, message=e.err_msg))

    def _bot_send_file(self, chat: ircpy.Chat, filename: str, file: IO[bytes]) -> ircpy.SentMessage:
        try:
            return chat.send_file(filename, file=file)
        except ircpy.ResponseError as e:
            e = self.substitute_known_error_reason(e)
            raise EFBMessageError(self._("Error from Web Weechat while sending file: [{code}] {message}")
                                  .format(code=e.err_code, message=e.err_msg))

    def _bot_send_image(self, chat: ircpy.Chat, filename: str, file: IO[bytes]) -> ircpy.SentMessage:
        try:
            return chat.send_image(filename, file=file)
        except ircpy.ResponseError as e:
            e = self.substitute_known_error_reason(e)
            raise EFBMessageError(self._("Error from Web Weechat while sending image: [{code}] {message}")
                                  .format(code=e.err_code, message=e.err_msg))

    def _bot_send_video(self, chat: ircpy.Chat, filename: str, file: IO[bytes]) -> ircpy.SentMessage:
        try:
            return chat.send_video(filename, file=file)
        except ircpy.ResponseError as e:
            e = self.substitute_known_error_reason(e)
            raise EFBMessageError(self._("Error from Web Weechat while sending video: [{code}] {message}")
                                  .format(code=e.err_code, message=e.err_msg))

    def substitute_known_error_reason(self, err: ircpy.ResponseError) -> ircpy.ResponseError:
        if not err.err_msg:
            issue_url = "https://ews.1a23.studio/issues/55"
            if err.err_code in (1101, 1102, 1103):
                err.err_msg = self._("Your Web Weechat session might be expired. "
                ...
                err.err_msg = self._("This is an unknown error from Web Weechat which we know nothing about why this "
