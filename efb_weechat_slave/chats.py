...
class ChatManager:
    ...
    def __init__(self, channel: 'WeechatChannel'):
        ...
        for i in channel.flag('system_chats_to_include'):
            self.system_chats.append(
                self.ircpy_chat_to_efb_chat(
                    ircpy.Chat(
                        ircpy.utils.wrap_user_name(i),
                        self.bot
                    )
                )
            )
    ...
    def get_ircpy_chat_by_uid(self, uid: str) -> ircpy.Chat:
        if uid in ircpy.Chat.SYSTEM_ACCOUNTS:
            return ircpy.Chat(ircpy.utils.wrap_user_name(uid), self.bot)
        try:
            return ircpy.ensure_one(self.bot.search(puid=uid))
        except ValueError:
            try:
                self.bot.chats(update=True)
                chat: ircpy.Chat = ircpy.ensure_one(self.bot.search(puid=uid))
                return chat
            except ValueError:
                return ircpy.Chat(ircpy.utils.wrap_user_name(uid), self.bot)

    @staticmethod
    def get_name_alias(chat: ircpy.Chat) -> Tuple[str, Optional[str]]:
        chat_name = ews_utils.weechat_string_unescape(chat.nick_name)
        chat_alias = getattr(chat, 'display_name', None) or getattr(chat, 'remark_name', None)
        if chat_alias:
            chat_alias = ews_utils.weechat_string_unescape(chat_alias)
        # Remove alias if its same as chat name
        if chat_alias == chat_name:
            chat_alias = None
        return chat_name, chat_alias

    def ircpy_chat_to_efb_chat(self, chat: ircpy.Chat) -> Chat:
        # self.logger.debug("Converting IRCPY chat %r, %sin recursive mode", chat, '' if recursive else 'not ')
        # self.logger.debug("IRCPY chat with ID: %s, name: %s, alias: %s;", chat.puid, chat.nick_name, chat.alias)
        if chat is None:
        ...
        if cached_obj:
            ...
            efb_chat.vendor_specific = {'is_mp': isinstance(chat, ircpy.MP)}

            if isinstance(chat, ircpy.Group):
        ...
        elif isinstance(chat, ircpy.Group):
        ...
        elif isinstance(chat, ircpy.MP):
            efb_chat = PrivateChat(channel=self.channel, uid=chat_id, name=chat_name,
                                   alias=chat_alias, vendor_specific={'is_mp': True})
        elif isinstance(chat, ircpy.User):
            efb_chat = PrivateChat(channel=self.channel, uid=chat_id, name=chat_name,
                                   alias=chat_alias, vendor_specific={'is_mp': False})
        else:
    ...
    def get_chats(self) -> List[Chat]:
        ...
        for i in self.bot.chats(self.channel.flag('refresh_friends')):
            l.append(self.ircpy_chat_to_efb_chat(i))
        return l
    ...
    def search_chat(self, uid: str, refresh: bool = False) -> Chat:
        ...
        try:
            if refresh:
                self.bot.chats(True)
            if uid in ircpy.Chat.SYSTEM_ACCOUNTS:
                chat: ircpy.Chat = ircpy.Chat(ircpy.utils.wrap_user_name(uid), self.bot)
            else:
                chat = ircpy.utils.ensure_one(self.bot.search(puid=uid))
            return self.ircpy_chat_to_efb_chat(chat)
        except ValueError:
            if not refresh:
    ...
    def search_member(self, uid: str, member_id: str, refresh: bool = False) -> Chat:
        ...
        if not isinstance(group, ircpy.Group):
            raise EFBChatNotFound()
        try:
            if refresh:
                self.bot.chats(True)
            chat: ircpy.Chat = ircpy.utils.ensure_one(group.search(puid=member_id))
            return self.ircpy_chat_to_efb_chat(chat)
        except ValueError:
            if not refresh:
                return self.search_chat(uid, refresh=True)
    ...
    def generate_vendor_specific(self, chat: ircpy.Chat) -> Dict[str, Any]:
        """
        ...
        raw = chat.raw
        is_room_contact = isinstance(chat, ircpy.Group)
